...
function (_super) {
  ...
  SankeyView.prototype.render = function (seriesModel, ecModel, api) {
    ...
    graph.eachEdge(function (edge) {
      ...
      // [Display value in sankey chart]
      
      const curveBoudingRect = curve.getBoundingRect()
      const valueText = new ZRText()

      valueText.useStyle({
        x: curveBoudingRect.x + curveBoudingRect.width - 30,
        y: y2 + curve.shape.extent/2 - 5,
        text: edgeModel.option.value,
      })

      group.add(valueText);

      // [END Display value in sankey chart]
